...
module restoring_divider #(
    ...
    logic [WIDTH:0] trial_remainder;

    always_comb begin

        shifted_remainder =
            {remainder_reg[WIDTH-1:0], dividend_reg[WIDTH-1]};
        ...
        if (rst) begin
            ...
            if (start) begin
                ...
                if (count == WIDTH-1) begin
                    ...
                    if (shifted_remainder >= {1'b0, divisor_reg}) begin
                        remainder <= trial_remainder[WIDTH-1:0];
                    end
                    else begin
                        remainder <= shifted_remainder[WIDTH-1:0];
                    end
